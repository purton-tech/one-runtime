...
MODE="${1:-}"
TMUX_CONFIG="/tmp/nails-tmux.conf"
DEBUG="${nails_TMUX_DEBUG:-0}"

on_error() {
  local exit_code="$?"
  echo "[nails-tmux] failed at line ${BASH_LINENO[0]} with exit ${exit_code}" >&2
  exit "$exit_code"
}
...
start_code_tmux() {
  ...
  tmux select-pane -t "$session:0.1" -T "nails"
...
start_site_tmux() {
  local session="agent-nails-com"
  local site_root="${REPO_ROOT}/crates/agent-nails-com"
...
if in_devcontainer; then
  ...
  exec devcontainer exec --workspace-folder "$REPO_ROOT" bash -lc "cd '$REPO_ROOT' && ./nails-tmux.sh '$MODE'"
fi
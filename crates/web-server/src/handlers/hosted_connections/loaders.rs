...
pub async fn loader_docs(_: routes::public_connect::Docs) -> Html<String> {
    Html(web_ui::public_connect::docs::page())
}

pub async fn loader_openapi_json(_: routes::public_connect::OpenApi) -> Response {
    let payload = json!({
        "openapi": "3.1.0",
        "info": {
            "title": "One Runtime Public Integrations API",
            "version": "1.0.0",
            "description": "Public REST endpoints for listing connectable integrations and creating hosted popup sessions."
        },
        "servers": [
            { "url": "/" }
        ],
        "components": {
            "securitySchemes": {
                "ApiKeyBearer": {
                    "type": "http",
                    "scheme": "bearer",
                    "description": "Org API key in the form oru_<prefix>_<secret>"
                }
            },
            "schemas": {
                "Integration": {
                    "type": "object",
                    "required": ["id", "slug", "name", "description", "status", "supported_auth_types"],
                    "properties": {
                        ...
                        "supported_auth_types": {
                            "type": "array",
                            "items": { "type": "string", "enum": ["api_key"] }
                        }
                    }
                },
                "IntegrationListResponse": {
                    "type": "object",
                    "required": ["end_user_id", "integrations"],
                    "properties": {
                ...
                "CreateHostedConnectionSessionRequest": {
                    "type": "object",
                    "required": ["integration_slug", "end_user_id"],
                    "properties": {
                        "integration_slug": { "type": "string" },
                        "end_user_id": { "type": "string" },
                        "end_user_name": { "type": ["string", "null"] },
                        "end_user_email": { "type": ["string", "null"] },
                        "suggested_connection_name": { "type": ["string", "null"] }
                    }
                },
                "CreateHostedConnectionSessionResponse": {
                    "type": "object",
                    "required": ["session_token", "connect_url"],
                    "properties": {
                        "session_token": { "type": "string" },
                        "connect_url": { "type": "string", "format": "uri-reference" }
                    }
                },
                "ErrorResponse": {
                    "type": "object",
        ...
        "paths": {
            "/api/public/integrations": {
                "get": {
                    "summary": "List connectable integrations for an end user",
                    "security": [{ "ApiKeyBearer": [] }],
                    "parameters": [
                        {
                            "in": "query",
                            ...
                            "required": true,
                            "schema": { "type": "string" },
                            "description": "Customer-defined end-user identifier used to derive saved connection status."
                        }
                    ],
                    "responses": {
                        "200": {
                            "description": "Available integrations with derived connection status.",
                            "content": {
                                "application/json": {
                ...
                "options": {
                    "summary": "CORS preflight for public integrations API",
                    "responses": { "204": { "description": "CORS headers only." } }
                }
            },
            "/api/public/hosted-connection-sessions": {
                "post": {
                    "summary": "Create a hosted popup session",
                    "security": [{ "ApiKeyBearer": [] }],
                    "requestBody": {
                        "required": true,
                        "content": {
                            "application/json": {
                                "schema": { "$ref": "#/components/schemas/CreateHostedConnectionSessionRequest" }
                            }
                        }
                    },
                    "responses": {
                        "200": {
                            "description": "Popup session created.",
                            "content": {
                                "application/json": {
                                    "schema": { "$ref": "#/components/schemas/CreateHostedConnectionSessionResponse" }
                                }
                            }
                        },
                        "401": {
                ...
                "options": {
                    "summary": "CORS preflight for hosted popup session creation",
                    "responses": { "204": { "description": "CORS headers only." } }
                }
            }
        }
    ...
    public_json_response(payload, StatusCode::OK)
}
